...
public class VoteWitnessActuatorTest {
  ...
  public void balanceNotSufficient() {
    ...
    try {
      actuator.validate();
      actuator.execute(ret);
      fail("The total number of votes[" + 1000000 + "] is greater than the sonicxPower["
          + balanceNotSufficientCapsule.getSonicxPower() + "]");
    } catch (ContractValidateException e) {
      Assert.assertEquals(0, dbManager.getAccountStore()
          .get(ByteArray.fromHexString(OWNER_ADDRESS_BALANCENOTSUFFICIENT)).getVotesList().size());
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert
          .assertEquals("The total number of votes[" + 1000000 + "] is greater than the sonicxPower["
              + balanceNotSufficientCapsule.getSonicxPower() + "]", e.getMessage());
      witnessController.updateWitness();
      WitnessCapsule witnessCapsule = witnessController
          .getWitnesseByAddress(StringUtil.hexString2ByteString(WITNESS_ADDRESS));
      Assert.assertEquals(10, witnessCapsule.getVoteCount());
