...
public class TransferActuator extends AbstractActuator {
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (Arrays.equals(toAddress, ownerAddress)) {
      throw new ContractValidateException("Cannot transfer sox to yourself.");
    }
    ...
    try {
      ...
      //after ForbidTransferToContract proposal, send sox to smartContract by actuator is not allowed.
      if (dbManager.getDynamicPropertiesStore().getForbidTransferToContract() == 1
              && toAccount != null
              && toAccount.getType() == AccountType.Contract) {
        throw new ContractValidateException("Cannot transfer sox to smartContract.");
      }
