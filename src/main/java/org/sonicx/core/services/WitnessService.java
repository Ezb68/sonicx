...
import org.sonicx.common.backup.BackupManager;
import org.sonicx.common.backup.BackupManager.BackupStatusEnum;
import org.sonicx.common.backup.BackupServer;
...
public class WitnessService implements Service {
   ...
   * Construction method.
   */
  public WitnessService(Application sonicxApp, SonicxApplicationContext context) {
  ...
  private BlockProductionCondition tryProduceBlock() throws InterruptedException {
    ...
    if (this.needSyncCheck) {
      ...
      if (nexSlotTime > now) { // check sync during first loop
        ...
        logger.debug("Not sync ,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
            new DateTime(now),
            new DateTime(this.sonicxApp.getDbManager().getDynamicPropertiesStore()
                .getLatestBlockHeaderTimestamp()),
            this.sonicxApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
            this.sonicxApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
    ...
    try {
      ...
      synchronized (sonicxApp.getDbManager()) {
        ...
        if (slot == 0) {
          logger.info("Not time yet,now:{},headBlockTime:{},headBlockNumber:{},headBlockId:{}",
              new DateTime(now),
              new DateTime(
                  this.sonicxApp.getDbManager().getDynamicPropertiesStore()
                      .getLatestBlockHeaderTimestamp()),
              this.sonicxApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderNumber(),
              this.sonicxApp.getDbManager().getDynamicPropertiesStore().getLatestBlockHeaderHash());
  ...
  private BlockCapsule generateBlock(long when, ByteString witnessAddress,
    ...
    return sonicxApp.getDbManager().generateBlock(this.localWitnessStateMap.get(witnessAddress), when,
