...
package org.sonicx.common.runtime.vm.program;
...
import static org.sonicx.common.runtime.utils.MUtil.transfer;
import static org.sonicx.common.runtime.utils.MUtil.transferAllToken;
import static org.sonicx.common.utils.BIUtil.isPositive;
import static org.sonicx.common.utils.BIUtil.toBI;
import static org.sonicx.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.sonicx.common.runtime.vm.EnergyCost;
import org.sonicx.common.runtime.vm.MessageCall;
import org.sonicx.common.runtime.vm.OpCode;
import org.sonicx.common.runtime.vm.PrecompiledContracts;
import org.sonicx.common.runtime.vm.VM;
import org.sonicx.common.runtime.vm.VMConstant;
import org.sonicx.common.runtime.vm.program.invoke.ProgramInvoke;
import org.sonicx.common.runtime.vm.program.invoke.ProgramInvokeFactory;
import org.sonicx.common.runtime.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.sonicx.common.runtime.vm.program.listener.CompositeProgramListener;
import org.sonicx.common.runtime.vm.program.listener.ProgramListenerAware;
import org.sonicx.common.runtime.vm.program.listener.ProgramStorageChangeListener;
import org.sonicx.common.runtime.vm.trace.ProgramTrace;
import org.sonicx.common.runtime.vm.trace.ProgramTraceListener;
import org.sonicx.common.storage.Deposit;
import org.sonicx.common.utils.ByteUtil;
import org.sonicx.common.utils.FastByteComparisons;
...
import org.sonicx.protos.Protocol.SmartContract.Builder;
...
public class Program {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertToSonicxAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = convertToSonicxAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowSvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all sox failed in suicide: %s", e.getMessage());
        }
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
      boolean isCreate2) {
    byte[] senderAddress = convertToSonicxAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertToSonicxAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = convertToSonicxAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = convertToSonicxAddress(getContractAddress().getLast20Bytes());
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertToSonicxAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(convertToSonicxAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] sonicxAddr = convertToSonicxAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(sonicxAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(sonicxAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(sonicxAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(convertToSonicxAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(convertToSonicxAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertToSonicxAddress(getContractAddress().getLast20Bytes()), key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(convertToSonicxAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress = convertToSonicxAddress(this.getCallerAddress().getLast20Bytes());
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(convertToSonicxAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(SonicxException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(SonicxException e) {
      return new PrecompiledContractException(e.getMessage());
    }
